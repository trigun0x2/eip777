...
contract ReferenceToken is Owned, erc20_token, erc777_token, EIP820Implementer {
    ...
    /// @notice Helper function that checks for erc777_tokenHolder on the recipient and calls it.
    ...
    function callRecipient(
        ...
        address recipientImplementation = interfaceAddr(_to, "erc777_tokenHolder");
        if (recipientImplementation != 0) {
            erc777_tokenHolder(recipientImplementation).tokensReceived(
    ...
    /// @notice Helper function that checks for erc777_tokenHolder on the sender and calls it.
    ///  May throw according to `_preventLocking`
    /// @param _from The address holding the tokens being sent
    /// @param _to The address of the recipient
    /// @param _value The amount of tokens to be sent
    /// @param _userData Data generated by the user to be passed to the recipient
    /// @param _operatorData Data generated by the operator to be passed to the recipient
    /// @param _preventLocking `true` if you want this function to throw when tokens are sent from a contract not
    ///  implementing `erc777_tokenHolder`.
    ///  ERC777 native Send functions MUST set this parameter to `true`, and backwards compatible ERC20 transfer
    ///  functions SHOULD set this parameter to `false`.
    function callSender(
        ...
        address senderImplementation = interfaceAddr(_from, "erc777_tokenHolder");
        if (senderImplementation != 0) {
            erc777_tokenHolder(senderImplementation).tokensToSend(
                _operator, _from, _to, _value, _userData, _operatorData);
        } else if (_preventLocking) {
            require(isRegularAddress(_from));
        }
    }
}
